	...
	public static void cmdParameterProcessor(String... args) {
		...
		if (areFormatted) {
			...
			if (!areManyOccurrences) {
				...
				System.out.println("Many occurrences for same arguments");
				performAction();
			}
		} else {
			System.out.println("The cmd arguments are not formatted");
			performAction();
		}
	}
...
class Help implements CmdOption {

	@Override
	public void execute() {
		System.out.println("THis is the usage for --help");
	}
}

class Debug implements CmdOption {
	@Override
	public void execute() {
		System.out.println("Set debug to disable");
	}
}

class OptionsProcessor {
	public static void main(String... args) throws InstantiationException, IllegalAccessException {
		Class classHelp = Help.class;
		Class classDebug = Debug.class;
		CmdOption referHelp = Help.class.newInstance();
		CmdOption referDebug = Debug.class.newInstance();
		referHelp.execute();
		referDebug.execute();

		System.out.println();

		var help = new CmdOption() {
			@Override
			public void execute() {
				System.out.println("THis is the usage for --help");
			}
		...
		CmdOption debug = () -> System.out.println("Set debug to disable");
		...
		CmdOption helpClass = new Help();
		CmdOption debugClass = new Debug();
		doSomething(helpClass);
		doSomething(debugClass);

		System.out.println();

		Consumer helpCons = System.out::println;
		...
		var res = helpFunc.apply("THis is the usage for --help");
		System.out.println(res);

	}

	private static void doSomething(CmdOption option) {
		option.execute();
	}
}
